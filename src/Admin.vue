			...
			<form @submit.prevent="save()">
				<div class="row">
						...
						<div class="row">
							...
							<div class="col-12">
								<div class="form-group">
									...
									<textarea type="text" class="form-control" name="content" id="content" v-model="content" rows="7"></textarea>
								</div>
							</div>
						</div>
					</div>
					<div class="col-12 col-sm-4">
						...
						<div class="form-group mt-3">
							<input type="file" @change="updatePreview($event)" ref="featured_image" class="form-control-file" />
						</div>
					</div>
				...
				<div class="row">
					<div class="col-12">
						...
						<button class="btn btn-secondary float-right" @click.prevent="cancel()">Cancel</button>
					</div>
				</div>
			</form>
		</div>
		<div v-if="posts.length" class="col-12 mt-5 pt-3 pb-3 bg-white from-wrapper">
			<h3>Posts</h3>
			<hr>
			<PostItem @delete="deletePost" @edit="getPost" v-for="(post, index) in posts" :key="post.post_id" :index="index" :post="post" :imgPlaceholder="imgPlaceholder"></PostItem>
		</div>
	</div>
</template>
	...
	export default {
		...
		methods: {
			...
			update() {
				...
				if(this.$refs.featured_image.files.length)
					form.append('featured_image', this.$refs.featured_image.files[0]);


				this.$api.post('/blog/update/' + this.post_id, form)
					.then(res => {
